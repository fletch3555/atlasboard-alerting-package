...
var async = require('async'),
    ...
    PagerDuty = require('./lib/PagerDuty.js');
...
module.exports = {
  ...
  onRun: function (config, dependencies, jobCallback) {
    ...
    var pd = new PagerDuty(config.apikey, {
      async: async,
    ...
    /*
     ...
     It is a good idea to cover this with unit tests as well (see test/pagerduty file)
     ...
     Have a look at test/pagerduty for an example of how to unit tests this easily by mocking easyRequest calls

     */

    pd.getAllUnresolvedIncidents({ sortfield: 'status' }, function(err, body) {
      if (!err) {
        jobCallback(err, {
          title: config.title,
